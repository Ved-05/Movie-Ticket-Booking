...
def add_money_and_book_ticket(name, email, showID):
    try:
        ...
        if (show_details_after_booking.json()['seats_available'] == show_details_before_booking.json()[
            'seats_available'] and old_wallet_balance == get_wallet(new_userid).json()['balance']):
            print("Test passed")
        else:
            print("Test failed")
    except:
