...
def add_money_and_check_detail(name, email, showID, walletAmount):
    try:
        ...
        if ((old_wallet_balance - new_wallet_balance == show_details_after_booking.json()['price'] * 10) and (
                show_details_before_booking.json()['seats_available'] - show_details_after_booking.json()[
            'seats_available'] == 10)):
            print("Test Passed")
        else:
            print("Test Failed")
